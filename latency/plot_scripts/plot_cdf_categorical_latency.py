...
for app, filename in files.items():
    ...
    if "delta_1" not in df.columns:
        print(f"[WARN] delta_1 not found in {filename}")
        continue

    latency_ms = pd.to_numeric(
        df["delta_1"].dropna(), errors="coerce"
    ).values
    ...
    all_apps = list(files.keys())

    # Generate 14 distinct colors
    colors = cm.get_cmap('tab20', len(all_apps))
    ...
    axs.plot(x, cdf_percent, label=app, linewidth=2, color=color_map[app])
...
if not any_plotted:
    ...
    axs.set_xlabel("Categorical classification Inference Time (µs)", fontsize=20)
